...
export const loader = async () => {
  const statusList = await prisma.reportStatus.findMany({
...
const ReportStatusIndexRoute = () => {
  const { statusList } = useLoaderData<typeof loader>()
          ...
          statusList.map(status => (
            ...
            >
              <div className='flex flex-1 flex-col px-2 py-6'>
                <p className='m-auto text-xl font-bold text-orange-400'>{status.name}</p>
                <dl className='mt-2 flex flex-grow flex-col justify-between gap-2'>
                  <dt className='font-semibold text-gray-100'>Display Name:</dt>
                  <dd className='ml-2 text-sm text-gray-300'>{status.displayName}</dd>
                  <dt className='font-semibold text-gray-100'>Description:</dt>
                  <dd className='ml-2 text-sm text-gray-300'>{status.description}</dd>
                </dl>
              </div>
              <div>
...
export default ReportStatusIndexRoute
